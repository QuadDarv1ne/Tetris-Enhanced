...
Улучшения по запросу пользователя:
 - Фоновая музыка (можно указать плейлист mp3)
...
Положите свои mp3 (перечисленные вами) в ту же папку, что и этот скрипт
или пропишите пути в MUSIC_FILES.

Запуск:
    pip install pygame
    python tetris_enhanced.py

Автор: обновлённый вариант для пользователя
"""
...
# Music files (put your mp3 files here, filenames you gave)
MUSIC_FILES = [
...
def draw_mini(surf, origin_x, origin_y, kind: Optional[str], small):
    box = pygame.Rect(origin_x, origin_y, PANEL_W-10, 60)
...
def pause_menu(screen, clock, font, small, audio: AudioManager, state: GameState):
    ...
    w = 420; h = 360
    rx = MARGIN + (PLAY_W - w)//2
    ry = MARGIN + (PLAY_H - h)//2
...
def run():
    ...
    font = pygame.font.SysFont('consolas,dejavusansmono,menlo,monospace', 36, bold=True)
    small = pygame.font.SysFont('consolas,dejavusansmono,menlo,monospace', 18)
    ...
    while True:
        ...
        if state.game_over:
            screen.fill(BG)
            draw_grid(screen, MARGIN, MARGIN, state)
            if state.current:
                draw_piece(screen, MARGIN, MARGIN, state.current)
        ...
        if state.current:
            ...
            draw_piece(screen, MARGIN, MARGIN, ghost, ghost=True)
            draw_piece(screen, MARGIN, MARGIN, state.current)
